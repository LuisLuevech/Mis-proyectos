...
// LOS CAÑOS
// Octubre 2018 Version 2.3
//
// www.luevech.com  
// La version 2.2 activa la parte metereologica cuando los segundos
// son igual a cero
// La version 2.3 solo activa la pantalla LCD (meteo) cuando al activar el pulsador y durante 7 seg.
...
char* dia[]={"Lun","Mar","Mie","Jue","Vie","Sab","Dom"};
int valdia;
int pinActivar=19;
volatile int pinDc=12;


void setup() {
  pinMode(pinActivar, INPUT_PULLUP);
  pinMode(pinDc,OUTPUT);
  attachInterrupt(digitalPinToInterrupt(pinActivar),meteo,LOW);
  ...
  myOLED.clrScr();
  myclock.begin();
  myclock.setTime(20,27,55);
  myclock.setDate(23,10,2018);
  myclock.setDOW(2);
...
void loop() {
  ...
  myOLED.setFont(SmallFont);
  if (myclock.getDOWStr(FORMAT_SHORT)=="Mon"){valdia=1;}
  if (myclock.getDOWStr(FORMAT_SHORT)=="Tue"){valdia=2;}
  if (myclock.getDOWStr(FORMAT_SHORT)=="Wed"){valdia=3;}
  if (myclock.getDOWStr(FORMAT_SHORT)=="Thr"){valdia=4;}
  if (myclock.getDOWStr(FORMAT_SHORT)=="Fri"){valdia=5;}
  if (myclock.getDOWStr(FORMAT_SHORT)=="Sat"){valdia=6;}
  if (myclock.getDOWStr(FORMAT_SHORT)=="Sun"){valdia=7;}
  myOLED.print(dia[valdia-1], 10, 50);
...
void meteo(){ 
  ...
  delay(1000);delay(1000);delay(1000);delay(1000);delay(1000);delay(1000);delay(1000);
